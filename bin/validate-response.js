...
const validateResponse = (response, expectStatus, expectBody, expectHeader) => {
    let out = '';
    const validStatus = expectStatus && response.statusCode ? expectStatus === response.statusCode : response.statusCode < 400;

    const validResponse = expectBody && response.body ? JSON.stringify(expectBody) === JSON.stringify(response.body) : response.body !== undefined;

    const validHeader = expectHeader && response.headers ? deepEqual(Object.assign(expectHeader, response.headers), response.headers) : true;
